    ...
    <header id="nav">
        <div class="nav--list">
            ...
            <a href="<?= Url::toRoute(['/site/index']) ?>">
                <h3 id="logo"><span class="logomask">MACKAPA</span></h3>
            </a>
        </div>

        <label class="cointotal">Всего: <?= $coinsStreamModel->coins ?></label>

        <div id="nav__links">
            <button id="chat__button"><svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" fill="#ede0e0" clip-rule="evenodd">
    ...
    <main class="container">
        <div id="room__container">
            ...
            <section id="stream__container">
                ...
                <div class="paycoins">
                    <label onclick="playCoins()" class="coin" id="50coins" name="<?= $_GET['room'] ?>" for="<?= $_GET['author'] ?>" data-url="<?= Url::toRoute(["/webrtc/default/coins"]) ?>">Каждое нажатие отправит автору стрима 50</label>
                    <script>
                        $(document).ready(function() {
                            $("#50coins").bind("click", function(event) {
                                var author = $(this).attr("for");
...
<?php if (Stream::find()->where(['user_id' => Yii::$app->user->identity->id])->andWhere(['task_id' => $_GET['room']])->one()) : ?>
                    <a onclick="alert('Удалить комнату? Это действие остановит стрим и перенаправит вас на главную страницу')" class="delroom" href="<?= Url::toRoute(['default/delroom', 'myid' => Yii::$app->user->identity->id, 'taskid' => $_GET['room']]) ?>">✕</a>
                <?php endif; ?>
